...
class FindLocationResultTest {
    ...
    void creatingFoundResultWithEmptyLocationsAndEmptyReferencesThrowsException() {
        assertThrows(IllegalArgumentException.class, () -> FindLocationResult.createFoundResult(Collections.emptyList(), Collections.emptyList()));
    }

    @Test
    void statusIsFoundForLocationResult() {
        Location location = mock(Location.class);
        assertThat("status", FindLocationResult.createFoundResult(Collections.singletonList(location), Collections.emptyList()).getStatus(), is(FindLocationResult.Status.FOUND));
    }

    @Test
    void statusIsFoundForLocationReferenceResult() {
        assertThat("status", FindLocationResult.createFoundResult(Collections.emptyList(), Collections.singletonList(new LocationReference(URI.create("http://example.com/1234"), Instant.now()))).getStatus(), is(FindLocationResult.Status.FOUND));
    }
